...
export class TabManage {
    ...
    constructor() {
        ...
        if (this.headerElement && this.sidebarElement && this.mainElement && this.footerElement) {
            ...
            sideBarViewChangeButton.addEventListener("click", () => {
                if (!this.sidebarElement)
                    return;
                if (this.sidebarElement.classList.contains("default"))
                    this.sidebarElement.classList.remove("default");
                else
                    this.sidebarElement.classList.add("default");
                const activeSideBarButton = this.sidebarElement.getElementsByClassName("active")[0];
                if (activeSideBarButton)
                    positionMath(activeSideBarButton, this.sidebarElement, 80);
            });
        }
        function getSideButtons(element) {
            const tabList = [];
            for (const childElement of element.getElementsByClassName("tabList"))
                childElement.classList.forEach(className => {
                    if (className.includes("tabName"))
                        tabList.push({ name: className, element: childElement });
                });
            return tabList;
        }
        if (this.sidebarElement && this.mainElement) {
            ...
            for (const sideBar of sideBarTabList)
                for (const mainTab of mainTabList)
                    if (sideBar.name === mainTab.name)
                        sideBar.element.addEventListener("click", () => { this.changeWindow(sideBar.element, mainTab.element); });
        }
    }
    changeWindow(sideBar, mainTab) {
        if (this.sidebarElement && this.mainElement) {
            const activeSideBarButton = this.sidebarElement.getElementsByClassName("active")[0];
